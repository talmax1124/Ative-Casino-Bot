...
class MarriageAnniversaryManager {
    ...
    async sendAnniversaryNotifications(marriage, monthsMarried, todayStr) {
        try {
            const partner1DmSent = await this.sendDMToUser(marriage.partner1_id, marriage.partner2_id, marriage.partner2_name, monthsMarried);
            const partner2DmSent = await this.sendDMToUser(marriage.partner2_id, marriage.partner1_id, marriage.partner1_name, monthsMarried);
            ...
            logger.info(`💒 Anniversary notifications sent for marriage ${marriage.id} (${monthsMarried} months)`);
     ...
     */
    async sendDMToUser(userId, partnerId, partnerName, monthsMarried) {
        try {
            ...
            const dmMessage = `${anniversaryEmoji} **Happy ${monthsMarried}-Month Anniversary!** ${anniversaryEmoji}\n\n` +
            ...
            logger.debug(`✅ Anniversary DM sent to ${user.username} (${userId})`);
            return true;
