...
async function performMaxBetRemovalSafetyCheck() {
    ...
    try {
        ...
        if (healthData.houseEdge < 10) {
            safe = false;
            reasons.push(`House edge too low: ${healthData.houseEdge.toFixed(1)}% (need 10%+)`);
        }

        if (healthData.totalGames < 10000) {
            safe = false;
            reasons.push(`Insufficient data: ${healthData.totalGames.toLocaleString()} games (need 10,000+)`);
        }

        if (healthData.profitableGames < 0.8) {
            safe = false;
            reasons.push(`Too many losing games: ${(healthData.profitableGames * 100).toFixed(1)}% profitable (need 80%+)`);
        }
