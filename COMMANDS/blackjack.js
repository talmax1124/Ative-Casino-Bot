/**
 * 🚀 HYBRID ENGINE-POWERED BLACKJACK COMMAND
 * Combines original Canvas/imaging with new Engine system
...
// 🚀 HYBRID ENGINE SYSTEM - Analytics + Canvas Integration
...
async function createGameEmbed(game, user, showDealer = false, balance = null, economicIndicators = null, regulatedPayout = null) {
    ...
    // 🚀 Add ENGINE ANALYTICS to embed if available
    let footer = game.gameEnded ? 'Game completed' : 'Choose your action';
    if (economicIndicators && !game.gameEnded) {
        footer += ` • Engine Tier: ${economicIndicators.tier}`;
        if (economicIndicators.gameId) {
            footer += ` | ID: ${economicIndicators.gameId.slice(-8)}`;
        }
    }
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            // Create embed and table image with ENGINE ANALYTICS
            const engineIndicators = {
                tier: settings.tier,
                adjustedWinRate: settings.adjustedWinRate,
                gameId: engineGameId
            };
            const embed = await createGameEmbed(game, interaction.user, false, userBalance, engineIndicators);
            const actionRows = createGameButtons(userId, game);
            const tableImage = await createGameTableImage(game, false);

            // 🎴 PHASE 1: Show dramatic card dealing animation first
            const tierColors = getTierColors(settings?.tier || 'Bronze');
            const dealAnimation = createCardDealAnimation();
            
            // Base embed template for animation
            const animationTemplate = {
                title: `${LUXURY_ICONS.DIAMOND} ${interaction.user.displayName}'s Blackjack Table ${LUXURY_ICONS.DIAMOND}`,
                color: tierColors[0],
                fields: [
                    {
                        name: 'Bet Amount',
                ...
                footer: 'Cards are being dealt from the luxury deck...'
            };
            
            // Play the dramatic card dealing animation
            await playAnimation(interaction, dealAnimation, animationTemplate, 200);
            
            // 🎴 PHASE 2: Show final game state with cards
            ...
            // 📊 RECORD GAME START WITH ENGINE ANALYTICS
    ...
    handleBlackjackAction: async function(interaction, actionId) {
        ...
        logger.debug(`Blackjack action: activeSession found=${!!activeSession}, gameType=${activeSession?.gameType}, expected=${SMGameType.BLACKJACK}`);
            ...
            if (activeSession && activeSession.gameType === SMGameType.BLACKJACK) {
                ...
                const gameSessionData = activeGames.get(sessionId);
                game = gameSessionData?.game;
                logger.debug(`Blackjack action: sessionId=${sessionId}, sessionData found=${!!gameSessionData}, game found=${!!game}`);
            }
    ...
    endGame: async function(interaction, game, userId, guildId) {
        try {
            ...
            const endGameSessionData = activeGames.get(game.sessionId);
            if (!endGameSessionData || endGameSessionData.game !== game) {
            ...
            // 🎲 GENERATE ENGINE OUTCOME - Advanced analytics and adjustments
            const outcomeSessionData = activeGames.get(game.sessionId);
            const engineGameId = outcomeSessionData?.engineGameId;
            ...
            try {
                ...
                // Try interaction update with proper error handling
                if (interaction.replied || interaction.deferred) {
