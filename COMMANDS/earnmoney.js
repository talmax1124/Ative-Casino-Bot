/**
 * EarnMoney command - Available only to users with 10+ votes
...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('earnmoney')
        .setDescription('Claim all economy commands at once (requires 10+ votes)'),
    ...
    async execute(interaction) {
        ...
        try {
            ...
            // Check if user has 10+ votes
            const voteData = await dbManager.getUserVoteData(userId, guildId);
            if (!voteData || voteData.total_votes < 10) {
                const embed = new EmbedBuilder()
                    .setTitle('🔒 Command Locked')
                    .setDescription('This command requires **10 or more votes** to unlock!')
                    .addFields(
                        { name: '🗳️ Your Votes', value: (voteData?.total_votes || 0).toString(), inline: true },
                        { name: '🎯 Required', value: '10 votes', inline: true },
                        { name: '📋 How to Vote', value: 'Use `/vote` to vote on Top.GG', inline: false }
                    )
                    .setColor(0xFF6B6B)
            ...
            const embed = new EmbedBuilder()
                ...
                .setColor(0x00FF7F)
                .setThumbnail(interaction.user.displayAvatarURL())
                .setFooter({ text: '💰 EarnMoney Command • Exclusive to Voters' })
                .setTimestamp();
