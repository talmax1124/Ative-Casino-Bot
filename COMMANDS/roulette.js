/**
 * 🚀 HYBRID ENGINE-POWERED ROULETTE COMMAND
 * Combines original roulette mechanics with new Engine system
 * Best of both worlds: Classic gameplay + Advanced analytics!
 */
...
module.exports = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const engineGameResult = await GameEngine.startGame('roulette', userId, guildId, betAmount, {
                mode: selectedMode,
            ...
            await analyticsEngine.recordGameEvent(userId, guildId, 'roulette_start', {
                gameId: engineGameId,
                betAmount: betAmount,
                mode: selectedMode,
    ...
    async spinRoulette(interaction, game, userId, guildId) {
        try {
            ...
            // 🎯 PHASE 1: Text-based roulette spinning animation first
            const rouletteAnimation = createRouletteAnimation();
            const dramaticPauses = createDramaticPause();
            
            // Base embed template for animation
            const animationTemplate = {
                title: `${LUXURY_ICONS.DIAMOND} ${interaction.user.displayName}'s Luxury Roulette ${LUXURY_ICONS.DIAMOND}`,
                color: LUXURY_COLORS.GOLD,
                fields: [
                    {
                        name: 'Current Bet',
                        value: `${game.currentBet.type}: ${formatLuxuryCurrency(game.currentBet.amount)}`,
                        inline: true
                    },
                    {
                        name: 'Potential Payout',
                        value: formatLuxuryCurrency(game.currentBet.amount * (game.currentBet.multiplier || 2)),
                        inline: true
                    }
                ],
                footer: 'The wheel begins to spin with luxury precision...'
            };
            
            // Play the dramatic roulette spinning animation
            const allFrames = [...rouletteAnimation, ...dramaticPauses];
            await playAnimation(interaction, allFrames, animationTemplate, 160);
            
            // SECURITY FIX: Use only editReply after deferUpdate to avoid double reply
            let hasUpdated = false;
            
            // 🎯 PHASE 2: Try to load the spinning GIF for extended animation
            try {
